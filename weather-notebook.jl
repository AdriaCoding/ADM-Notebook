...
# ╔═╡ 1e925f60-0797-11ef-024d-f96b36f01881
using CSV, DataFrames, Plots, Random, Dates, Optimization, ComponentArrays, Lux, OptimizationOptimisers, DiffEqFlux,
    OrdinaryDiffEq, CSV, DataFrames, Statistics, Plots, DataDeps

# ╔═╡ c2d60525-3272-4c7c-9c27-52091f871b3f
md"""
## Precompile all packages (may take a while)
"""

# ╔═╡ f06aa198-2c8f-4d12-bb47-95f144b75d42
md"""
## Load the Delhi weather datasets
"""

# ╔═╡ e17a2d34-5199-4bf9-a09c-b9c0f84fb720
...
# ╔═╡ 502c22b6-9389-456c-a25b-87281bff76b7
begin
	df = download_data()
...
# ╔═╡ 5b0e9687-2440-43ed-a41a-c422b108691e
df[20,:]

# ╔═╡ dfb2435d-fdca-4f53-ab76-f2a87d8479da
md"""
## Plot of our weather data
"""

# ╔═╡ 06608f75-484d-4e52-9760-b646d3326358
...
# ╔═╡ 93c44091-ce11-43fe-bcc7-201172c3c932
plot_data(df)

# ╔═╡ 4f2e6abd-ab0a-4780-b55f-142653bc92a5
md"""
## Data handling 
"""

# ╔═╡ 6725379e-a5e2-46fa-9547-7c0f17837f93
md"""
In the following, we will normalize our weather data.

For some sample ``X\sim N(\mu, \sigma)``, function 'standarize' transforms it to ``Z\sim N(0,1)``.
"""

# ╔═╡ 7a81ae8e-cbe7-4512-b30f-de24167f481e
...
# ╔═╡ cc02a815-cfcf-4fb8-87d6-c30bf3137bbc
md"""
Function 'featurize' will return the normalized train and test data.

We take the averages of the data for every month, reducing the data size from 1576 to 52. Then, from this 52 new samples we take 20 to train our model, and the remaining 32 will be used for testing.
"""

# ╔═╡ 85c8a458-da2b-4950-89b8-47aff7367fdd
...
# ╔═╡ 1277215b-5ae1-4849-a5bf-fce4894da6be
t_train, y_train, t_test, y_test, (t_mean, t_scale), (y_mean, y_scale) = featurize(df)

# ╔═╡ 5bd13873-b92d-406c-bab0-08ec943201d4
md"""
The train dates are also transofrmed into normalized real values, ``t_{train}\sim N(0,1)``. 

The same transformation``\frac{ \cdot -\mu}{\sigma}`` normalizing the train dates is applied to the test dates, but do note that ``t_{test}\nsim N(0,1)``
"""

# ╔═╡ 0c03fd47-35a9-426d-9334-0661ea08e6fe
...
# ╔═╡ a22e1665-4e3d-47b2-b663-2b2c10681d8c
plot_features(t_train, y_train, t_test, y_test)

# ╔═╡ 904ddb7a-40f6-424b-9e3c-2ce6911a5644
md"""
## Neural ODE model
"""

# ╔═╡ 0290d241-cb8e-45f9-8ab1-608855eb8dd1
md"""
We define a dense neural networ for the vector field $f$ using `Lux`, and the whole Neural Ordinary Differential Equation is defined usign `DiffEqFlux`. Recall that the NODE model represents the solution of the initial value problem
...
# ╔═╡ 0a38fe1e-0ee4-4cd4-9aaa-6fb198d4b7dd
...
# ╔═╡ 465285b0-f72a-4a01-8d0b-6ff6c0a0b0f7
...
# ╔═╡ 5bed76b0-143e-4a72-b4dc-231f96d2b348
function train(t, y, obs_grid, maxiters, lr, rng, p = nothing, state = nothing; kwargs...)
    ...
    ps, losses = ComponentVector{Float32}[], Float32[]
    for k in obs_grid
...
# ╔═╡ 4374ad51-cd73-41bb-b629-89a64e3cd8a8
begin
	rng = MersenneTwister(123)
	obs_grid = 4:4:length(t_train) # train on an increasing amount of the first k obs
	maxiters = 150
	lr = 5e-3
end

# ╔═╡ 327d3b3d-ce31-40e5-8e2b-7e3f2b326cdb
# ╠═╡ disabled = true
#=╠═╡
lol = ComponentVector{Float32}[]
  ╠═╡ =#

# ╔═╡ 001f06af-5e42-458c-8d61-4620ec79ba22
# ╠═╡ disabled = true
#=╠═╡
push!(lol, copy(p))
  ╠═╡ =#

# ╔═╡ 86599d21-3823-45d8-9209-5a094cd248e5
# ╠═╡ disabled = true
#=╠═╡
push!(ps, p1)
  ╠═╡ =#

# ╔═╡ 848fac7c-ab82-4622-810e-3977324d7cf4
begin
...
# ╠═╡ disabled = true
#=╠═╡
p, _ = train_one_round(node, p_new, state_new, y_train, AdamW(lr), maxiters, rng)
  ╠═╡ =#

# ╔═╡ d0613972-e51f-4046-bfb0-2532e72ef347
#=╠═╡
begin
	p1 = copy(p)
	p2 = copy(p)
	ps = ComponentVector{Float32}[]
end
  ╠═╡ =#

# ╔═╡ 9554d0f8-80a6-4b47-b5b4-59e95b8c7eb3
# ╠═╡ disabled = true
#=╠═╡
begin
		node, p_new, state_new = neural_ode(t_train, size(y, 1))
        p === nothing && (p = p_new)
        state === nothing && (state = state_new)
...
manifest_format = "2.0"
project_hash = "96368e7a68bad9164dcb244aa63af00757efc685"
...
git-tree-sha1 = "433d5947cc45c570cc0f8de45f534e0c667354e7"
...
[[deps.LuxCore]]
deps = ["FastClosures", "Functors", "Random", "Setfield"]
git-tree-sha1 = "f799f3aa8599f79ed5e2c9fbaf74907c1ebe15ce"
...
[[deps.LuxLib]]
deps = ["ArrayInterface", "ChainRulesCore", "FastBroadcast", "FastClosures", "GPUArraysCore", "KernelAbstractions", "LinearAlgebra", "LuxCore", "Markdown", "NNlib", "Polyester", "PrecompileTools", "Random", "Reexport", "Statistics"]
git-tree-sha1 = "04ddd176f3f3e51ca52ecebcbbdef5f0233bb5b2"
...
[[deps.Optimization]]
deps = ["ADTypes", "ArrayInterface", "ConsoleProgressMonitor", "DocStringExtensions", "LinearAlgebra", "Logging", "LoggingExtras", "OptimizationBase", "Pkg", "Printf", "ProgressLogging", "Reexport", "SciMLBase", "SparseArrays", "TerminalLoggers"]
...
git-tree-sha1 = "6266f3365980f0a6a39426316595bc1254df239c"
...
git-tree-sha1 = "a7f777fff9cc15920e1e6c040c1e25b769760a8e"
